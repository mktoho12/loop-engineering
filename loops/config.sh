...
#
# 60分。実測（修正ループ11回）の分布は中央値 6.4分・最長 44.1分で、
# 正常に完了する実行が 44分かかった実績がある。30分にすると、
# 詰まった実行ではなく「時間のかかる正常な実行」を殺すことになる。
#
# 2700（45分）から引き上げた。2700 では実績の最長 44.1分との差が 54 秒しかなく、
# 実際に 2026-08-19 の Issue #143 が **2698 秒**（上限の2秒前）で殺された。
# 死ぬ場所が最悪で、fix.md は「レビュー → push・PR」の順のため
# 完成した作業が push されずローカルに埋もれ、その Issue が永久に
# スキップされ続ける（152回連続 SKIP の原因）。詳細は
# 06-recovering-a-stalled-loop.md を参照。
#
# なお launchd / systemd 側のタイムアウトはこれより大きくしておくこと
