...
#
# product-gap / ux は「既存コードの欠陥」ではなく「作られていないもの」を見る観点。
# 他の4観点は全て「今あるコードが正しいか」を問うており、そこを幾ら磨いても
# プロダクトは前に進まない。実際、テストが本体の5倍（2,051行 vs 402行）まで
# 育つ一方で、web は API を一度も呼んでおらず、CLAUDE.md の MVP スコープ
# （起票フォーム・地図・コメント・マッチング）は1つも実装されていなかった。
# 品質のループだけを回すと、この状態が「健全」に見えてしまう。
LENSES=(
