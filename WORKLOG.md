...
| **いまループは仕事をしているか** | ❌ **していない。2026-08-20 19:31 を最後に実作業ゼロ**（下記「⚠️ いま起きていること」） |
| **リポジトリ分離（8/22 の作業）** | ✅ 完了。ただし **Mac と em105 の main が分岐したまま** |

### 👉 次にやること

1. **em105 のループの詰まりを解く（8/20 から実作業ゼロ）**
   - `fix/issue-139` と `fix/issue-143` が push されないまま em105 のローカルに居座り、
     修正ループが 2026-08-20 19:31 以降 **152 回連続で「すべて着手済み」SKIP** している
   - 中身を見て push→PR にするか破棄する（doc `06` と**同じ形の再発**。手順はそこに書いてある）
   ```bash
   ssh em105-mktoho
   cd ~/work/ai/world-issue-tracker && git log --oneline main..fix/issue-139  # 中身を見てから判断
   ```
2. **em105 と origin の分岐を解消する** — em105 に未 push が 2 件（`df66f83` 発見ループの systemd unit、
   `ff7f687` 監視ループの誤報修正）、Mac 発の未取り込みが 2 件（`a4ca5d4` ループの汎用化、`9f8c126` WIT 資料の移設）。
   **em105 で `git pull --rebase` してから push** するのが素直
3. **WIT の PR #156 をマージする** — 8/22 の分離作業の WIT 側。ブランチ `chore/import-loop-docs`
   （マージは人間に委ねられたまま。https://github.com/EngineMaker/world-issue-tracker/pull/156 ）
4. **WIT のオープン Issue 5 件のうち 3 件が `needs-discussion`** — ループは触らない。
   人間がトリアージしないと、詰まりを解いても発見ループの HEAD が動かず SKIP し続ける

### ⚠️ いま起きていること（2026-08-22 12:xx 時点で実測）

3つのタイマーは正常に起動している（`fix` 毎時 0/30分、`watch` 15/45分、`discover` 11/41分ごろ）。
だが**全部 SKIP している**:

```
[2026-08-22 13:00:38] Issue #139 はブランチが既にあるのでスキップ
[2026-08-22 13:00:38] Issue #143 はブランチが既にあるのでスキップ
[2026-08-22 13:00:38] SKIP: オープン Issue は 2 件あるが、すべて着手済み
```

- `fix/issue-139` … em105 のローカルのみ。コミット1件（詳細ページテストの fetch スタブ修正）
- `fix/issue-143` … em105 のローカルのみ。コミット1件（起票者が自分の Issue を編集できるようにする）
- どちらも **remote には存在しない**＝ push されずに埋もれた作業
- `discover` は「HEAD が前回から変わっておらず、全観点を一巡済み」で SKIP（`75786ba1` のまま）
- 監視ループは動いているが、この状態を新しく報告していない
  （WIT Issue #136 `[loops:health]` が 8/20 に立っており `needs-discussion` のまま放置）

**これは doc `06-recovering-a-stalled-loop.md` に書いた 8/11〜8/18 の停止と同じ構造。**
「中身のあるブランチは自動回収されない」という既知の欠陥が、まだ直っていない。
